...
output:
  ...
  html_document:
    df_print: paged
subtitle: Lesson Plan 6 - Infiltration Week 2 of 3 completed
fontsize: 12pt
---
...
# 2. What is the maximum depth of water that can be stored in the top 
# 0.8 meters of a soil profile of Sue, which has a porosity of 0.4.  
# Determine the volumetric soil water content if the degree of 
# saturation is 35%.  

# Find vol_tot in cubic inches
depth_m      <- 0.8      # m
depth_cm      <- depth_m * 100      # 1cm = 1 m
area_sq_cm    <- 1         # square cm
vol_tot_cu_cm <- depth_cm * area_sq_cm    # cubic cm
rm(depth_m, depth_cm)

# Find the maximum volume of water that can be stored 
porosity       <- 0.4 # unitless
vol_void_cu_cm <- porosity * vol_tot_cu_cm # cubic_cm

# 2.1. find the maximum depth of water that can be stored 
vol_void_cm    <- vol_void_cu_cm/area_sq_cm 

# 2.2. find the amount of water in the soil at 35% saturation
perc_sat       <- 0.35  # unitless
Theta_v        <- perc_sat * porosity  # unitless

rm(area_sq_cm, perc_sat, porosity, vol_tot_cu_cm, vol_void_cu_cm)

# 3. A sample of Sue is placed in a weighing container with a mass of
# 20 grams.  The weighing container plus soil is 150 grams.  The 
# sample is then put into a drying oven for 24-hours at 104 C. 
# The sample plus tin is cooled and reweighed.  The final mass is 125 
# grams.  What is the water content by mass?

mass_tin            <-  20 # grams
mass_tin_soil_water <- 150 # grams
mass_tin_soil       <- 125 # grams

# Subtract off the mass of the tin.
mass_soil_water = mass_tin_soil_water - mass_tin # grams
mass_soil = mass_tin_soil - mass_tin # grams
mass_water = mass_soil_water - mass_soil # grams

rm(mass_tin, mass_tin_soil_water, mass_tin_soil, mass_soil_water)

# Find water content by mass
wat_cont_mass = 100* mass_water / mass_soil

rm(mass_water, mass_soil)

# 4. Density calculations
# The dry bulk density of a soil sample of Sue is 1.68 grams per cubic cm.
# The density of particles was measured to be 2.8 grams per cubic cm.  
# Find the porosity of the soil profile
# Find Gs of soil particles.  
# Find the bulk density of a saturated soil sample.

dens_dry_soil   <-  1.68  #  grams per cubic cm
dens_part  <- 2.8  #  grams per cubic cm
dens_water <- 1.0  #  grams per cubic cm

# 4.1. Use the equation for porosity above
porosity <- 1 - (dens_dry_soil/dens_part)

# 4.2. Specific gravity of the soil 
SpGrav_part <- dens_part / dens_water # unitless or  g/cc per g/cc

# 4.3. Find density of sat soil
mass_water_per_cc <- porosity * dens_water
dens_sat_soil <- dens_dry_soil + mass_water_per_cc # grams per cc

rm(dens_dry_soil, dens_part, dens_water)

# Determine the amount of water needed to increase soil water 
# content in the root zone (0.8 meters) of a soil named Sue from 
# WP to FC.  Figure 3.9 shows the wilting point of this type of soil is 1 inch per foot and the field capacity is 3.2 inches per foot.  

wilt_point  <- 1   # inches water per foot soil
field_cap   <- 3.2 # inches water per foot soil
PAW         <- field_cap - wilt_point

wat_need_in_per_ft <- field_cap - wilt_point # inch/foot
root_zone_ft     <- 0.8*3.14 # ft 
wat_need_rootzone <- wat_need_in_per_ft * root_zone_ft
```
```
